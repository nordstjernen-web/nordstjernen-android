...
# Firefox vendors (dav1d/opus/vorbis/ogg). Built STATIC (.a) like the rest of
# the iOS sysroot. H.264/HEVC/AAC are provided by the OS (VideoToolbox /
# AudioToolbox), so they are not built here.

dep_libogg() {
  local s; s="$(fetch_source libogg)"
  # Container framing used by Vorbis/Opus. Installs ogg.pc and the CMake package
  # config (OggConfig.cmake) that libvorbis's find_package(Ogg) needs.
...
dep_opus() {
  ...
  # Static here (BUILD_SHARED_LIBS=OFF from build_cmake); OPUS_HARDENING stays on
  # by default. No external assembler: SIMD is via compiler intrinsics.
  build_cmake "${s}" -DOPUS_BUILD_TESTING=OFF -DOPUS_BUILD_PROGRAMS=OFF
}

dep_dav1d() {
  local s; s="$(fetch_source dav1d)"
  # AV1 decoder (Firefox's). Drop the CLI tool, tests and examples. iOS is arm64
  # only, whose asm is assembled by clang, so nasm is not needed on this path.
  build_meson "${s}" \
